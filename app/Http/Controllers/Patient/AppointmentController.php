...
class AppointmentController extends Controller
{
    ...
    /**
     * Store new appointment and (optionally) create billing record.
     */
    public function store(Request $request)
    {
        ...
        // 💰 Optional direct billing (guarded + toggle)
        if (config('billing.enable_direct', true)) {
            try {
                ...
                \Log::info('✅ Consultation charge created (direct store)', [
                    'appointment_id' => $appointment->id,
                    'physician_id' => $appointment->physician_id
                ]);
            } catch (\Throwable $e) {
                \Log::error('❌ Consultation billing failed (direct store)', [
                    'appointment_id' => $appointment->id,
                    'error' => $e->getMessage()
                ]);
            }
    ...
    /**
     * Complete consultation - update appointment status and bill it.
     */
    ...
    public function completeConsultation(Request $request, $id)
    {
        try {
            ...
            // 🏥 Ensure an encounter exists for this appointment
            if (!$appointment->encounter_id) {
                $encounter = \App\Models\Encounter::create([
                    'patient_id' => $appointment->patient_id,
                    'encounter_number' => 'ENC' . str_pad((string) now()->timestamp, 8, '0', STR_PAD_LEFT),
                    'type' => 'OPD',
                    'status' => 'ACTIVE',
                    'department_id' => $appointment->department_id,
                    'attending_physician_id' => $appointment->physician_id,
                    'chief_complaint' => $appointment->chief_complaint ?? 'General consultation',
                    'admission_datetime' => now(),
                ]);

                $appointment->update(['encounter_id' => $encounter->id]);

                \Log::info('🆕 Encounter created for consultation', [
                    'encounter_id' => $encounter->id,
                    'appointment_id' => $appointment->id
                ]);
            } else {
                $encounter = \App\Models\Encounter::find($appointment->encounter_id);
                
                // If encounter doesn't exist, create a new one
                if (!$encounter) {
                    \Log::warning('🔄 Encounter not found, creating new one', [
                        'appointment_id' => $appointment->id,
                        'missing_encounter_id' => $appointment->encounter_id
                    ]);
                    ...
                    \Log::info('🆕 New encounter created for existing appointment', [
                        'encounter_id' => $encounter->id,
                        'appointment_id' => $appointment->id
                    ]);
                }
            ...
            // 💰 Direct billing logic (non-blocking, with trace)
            if (config('billing.enable_direct', true) && $encounter) {
                try {
                    $billingResult = $this->billingService->addConsultationCharge(
                        $encounter->id,
                        $appointment->physician_id,
                        'OPD'
                    );

                    \Log::info('✅ Consultation charge created (direct complete)', [
                        'appointment_id' => $appointment->id,
                        'encounter_id' => $encounter->id,
                        'physician_id' => $appointment->physician_id,
                        'billing_id' => $billingResult->id ?? null,
                    ]);

                    // 💰 Billing Trace Summary
                    $patientName = "{$appointment->patient->first_name} {$appointment->patient->last_name}";
                    $amount = $billingResult->amount ?? 0;
                    \Log::info("💰 Billing Trace → Appointment #{$appointment->id} | {$patientName} | Encounter #{$encounter->id} | Charge: OPD Consultation | KES {$amount} | Status: SUCCESS");
                } catch (\Throwable $e) {
                    \Log::error('❌ Consultation billing failed (direct complete)', [
                        'appointment_id' => $appointment->id,
                        'encounter_id' => $encounter->id ?? 'N/A',
                        'error' => $e->getMessage()
                    ]);
                    $encounterId = $encounter->id ?? 'N/A';
                    \Log::warning("💰 Billing Trace → Appointment #{$appointment->id} | Encounter #{$encounterId} | Charge: OPD Consultation | Status: FAILED");
                }
            } elseif (!$encounter) {
                \Log::warning('⚠️ Skipping billing - no encounter available', [
                    'appointment_id' => $appointment->id
                ]);
            }

            \Log::info('DEBUG: Regular Consultation Completed', [
                'appointment_id' => $appointment->id,
    ...
    /**
     * Complete consultation for an OPD appointment and bill it.
     */
    private function completeOpdConsultation($opdAppointment)
    {
        try {
            ...
            // 💰 Direct billing logic (non-blocking)
            if (config('billing.enable_direct', true)) {
                try {
                    $this->billingService->addConsultationCharge(
                        $opdAppointment->id,
                    ...
                    \Log::info('✅ OPD Consultation charge created (direct)', [
                        'opd_appointment_id' => $opdAppointment->id,
                        'physician_id' => $opdAppointment->physician_id
                    ]);
                } catch (\Throwable $e) {
                    \Log::error('❌ OPD Consultation billing failed (direct)', [
                        'opd_appointment_id' => $opdAppointment->id,
                        'error' => $e->getMessage()
                    ]);
                }
