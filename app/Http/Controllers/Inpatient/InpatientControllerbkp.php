...
class InpatientController extends Controller
{
        ...
        $recentPatients = DB::table('encounters')
            ...
            ->map(function ($p) {
                ...
                return [
                    ...
                    'diagnosis' => $p->chief_complaint ?? 'Not specified',
    ...
    private function handleAdmissionPost(Request $request)
    {
        ...
        try {
            $validated = $request->validate([
                ...
                'attending_physician_code' => 'nullable|string',
            ...
            // Resolve physician record using either id or code
            $physician = null;
            if (!empty($validated['attending_physician_id'])) {
                $physician = DB::table('physicians')->where('user_id', $validated['attending_physician_id'])->first();
            }
            if (!$physician && !empty($validated['attending_physician_code'])) {
                $physician = DB::table('physicians')->where('physician_code', $validated['attending_physician_code'])->first();
            }
            if (!$physician && $request->has('attending_physician')) {
                $physician = DB::table('physicians')->where('physician_code', $request->input('attending_physician'))->first();
            }

            if (!$physician) {
                throw ValidationException::withMessages([
                    'attending_physician' => 'Physician not found for provided identifier.'
                ]);
            }

            Log::debug('Admission validated', [
            ...
            // Create diagnosis only when icd10_code is present AND not empty
            ...
            Log::warning('Admission validation failed', [
                'errors' => $ve->errors(), 
                'payload_keys' => array_keys($request->all())
            ]);
            return response()->json([
    ...
    public function patients($id = null)
    {
        ...
        $patient = DB::table('encounters')
            ...
            ->leftJoin('beds', 'bed_assignments.bed_id', '=', 'beds.id')
            ->leftJoin('wards', 'beds.ward_id', '=', 'wards.wardid')
            ->leftJoin('physicians', 'encounters.attending_physician_id', '=', 'physicians.id')
    ...
    public function admitPatient(Request $request)
    {
        try {
            ...
            Log::info('🏥 Incoming admit-patient payload:', $request->all());
            
            $validated = $request->validate([
                'patient_id' => 'required|string|exists:patients,id',
                'bed_id' => 'required|integer|exists:beds,id',
                'attending_doctor_id' => 'required|string|exists:physicians,id',
            ...
            // Check if bed is available
            $bed = DB::table('beds')->where('id', $validated['bedId'])->first();
            if (!$bed || $bed->status !== 'available') {
            ...
            Log::info('Bed assignment created:', [
